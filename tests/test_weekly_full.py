...
def test_weekly_full_dedupes_multi_exchange_trade_calendar(tmp_path, monkeypatch):
    db_path = tmp_path / "test.duckdb"
    ...
    assert result.report["reference_start_date"] == "2026-06-01"
    assert result.report["compare_start_date"] == "2026-06-02"
...
def test_weekly_full_create_snapshot_does_not_self_compare(tmp_path, monkeypatch):
    db_path = tmp_path / "test.duckdb"
    _seed_db(db_path)
    monkeypatch.setattr(weekly_full, "REPORTS_DIR", tmp_path)
    ...
    assert result.report["summary"]["status"] == "snapshot_created"
    assert result.report["compare_report"] is None

    compare = weekly_full.run_weekly_full(
        as_of_date="2026-06-05",
        reference_days=3,
        compare_days=2,
        tables=["derived_daily_spine"],
        snapshot_prefix="snap_",
        output_prefix="weekly_compare",
        db_path=db_path,
    )

    assert compare.report["summary"]["status"] == "pass"
    assert compare.report["compare_report"]["summary"]["pass_table_count"] == 1


def test_weekly_full_create_snapshot_is_not_validation_pass(tmp_path, monkeypatch):
    db_path = tmp_path / "test.duckdb"
    _seed_db(db_path)
    monkeypatch.setattr(weekly_full, "REPORTS_DIR", tmp_path)
...
def test_weekly_full_can_auto_create_missing_snapshot(tmp_path, monkeypatch):
    db_path = tmp_path / "test.duckdb"
    _seed_db(db_path)
    monkeypatch.setattr(weekly_full, "REPORTS_DIR", tmp_path)
    ...
    result = weekly_full.run_weekly_full(
        ...
        tables=["derived_daily_spine"],
        snapshot_prefix="snap_",
        output_prefix="weekly_auto_snapshot",
        auto_create_missing_snapshot=True,
        db_path=db_path,
    )

    assert result.report["summary"]["status"] == "snapshot_created"
    assert result.report["missing_snapshot_tables"] == []
    assert result.report["snapshot_counts"] == {"derived_daily_spine": 2}
    assert result.report["compare_report"] is None
